-- Recent-projects picker (same data as project.nvim’s Telescope extension) with a
-- different default: <CR> cds into the project and opens a new empty buffer instead
-- of Telescope find_files. Other keymaps match project.nvim (f, b, s, r, w, d).
local M = {}
...
--- Open recent-projects Telescope; <CR> = cd + empty buffer (no find_files).
  ...
  pickers.new(opts, {
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      actions.select_default:replace(function()
        ...
        actions.close(prompt_bufnr)
        if project.set_pwd(entry.value, "telescope") then
          vim.cmd("enew")
        end
      end)
      return true
